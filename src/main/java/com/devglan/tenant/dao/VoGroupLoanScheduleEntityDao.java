...
public interface VoGroupLoanScheduleEntityDao extends JpaRepository<VoGroupLoanScheduleEntity, BigInteger>, JpaSpecificationExecutor<VoGroupLoanScheduleEntity> {
    ...
    @Modifying
    @Query(nativeQuery = true, value = "Update vo_group_loan_schedule set repaid=false, loan_paid=0, loan_os=principal_demand,last_paid_date=null where vo_mtg_uid=?1")
    void updateVoGroupLoanSchedule(BigInteger voMtgUID);
    ...
    @Query(nativeQuery = true, value = "Select * from vo_group_loan_schedule where cbo_id=?1 and loan_no=?2 and installment_no = ?3 and sub_installment_no=?4")
