...
public class DownloadBulkReportExcelWriter {
	String strDataExcelPath;
	private static Log log = LogFactory.getLog(DownloadBulkReportExcelWriter.class);		
	...
	public WorkBook PrepareExcel(String str_session_id, 
		...
		try {
			...
			objWorkBook.setText(0, 3, "Year : "+ selYear);
			
			
			
			lstdata = DataFetcher.getProcData("CALL sp_df_bulk_rept_download("+
												Utility.checkVal(rept_type)+", "+
												Utility.checkVal(selDist)+", "+
												Utility.checkVal(selYear)+
												")");
