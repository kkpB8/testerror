...
$(document).ready(function() {	
	...
	GetEditApprovedSummary($('#sel_yr').val());
	
});


/**
 * # To Get Summary For Approved Edit Request
 * @param year
 * @returns
 */

function GetEditApprovedSummary(year) {
	$('#div_data').html('');
	showModal();
	$.ajax({
		  url: "../../pop.EditApprovedSummryAll",
		  type: "get",
		  data: {
			  sel_yr: year,
		  },
...
function FetchLinkData(formid, mnthid, yr) {
  ...
  $('#tblApproveDetail').DataTable({
	    'ajax':  '../../pop.EditApprovedHistoryDetail?formid=' //# Data Fetch
...
$("#aLnkBack" ).click(function() {
	...
	if (G_BACK_TBL==2) {
		$('#divYr').show();
		GetEditApprovedSummary(G_BACK_YEAR);
		$("#aLnkBack").hide();
		G_BACK_TBL=1;
	}
	if (G_BACK_TBL==3) {
...
function prepareData(val) {
	...
	$.ajax({
		url: "../../pop.DataManagePopulatorHist",
		type: "get",
		data: {			
