...
/**
 * Function to show User Modules
 * @returns
 */
function prepareUserModules() {
	//console.log("here in prepareUserModules...");
	...
	$.ajax({
		...
		success: function(response) {
			...
			if (response == "N/A") {
				...
				drawUserModulesTableChart(obj_response);
			}
		},
		error: function(xhr) {
			bootbox.alert("Unable to get data.");
...
/**
 * Function to draw Table Chart for User Modules
 * @param obj_response
 * @returns
 */
function drawUserModulesTableChart(obj_response) {
	console.log(obj_response);	
	var tbl_data = '';
	tbl_data += '<table id="tbl_module" class="display" cellspacing="0" border=1 style="width:100%">';	
		tbl_data += '<thead>';
			tbl_data += '<th>Menu</th>';
			tbl_data += '<th>Sub Menu</th>';
			tbl_data += '<th>Group</th>';
			tbl_data += '<th>Sub Group</th>';
			tbl_data += '<th>Select</th>';
		tbl_data += '</thead>';
...
/**
 * Function to set User selected module
 * @param is_checked
 * @param menu_code
 * @param sub_menu_code
 * @param grp_code
 ...
 * @returns
 */
function setUserSelectedVal(is_checked, menu_code, sub_menu_code, grp_code, sub_grp_code) {
	//console.log(is_checked+"..."+menu_code+"..."+sub_menu_code+"..."+grp_code+"..."+sub_grp_code);
	if(is_checked) {
		arr_user_data.push(menu_code+"^"+sub_menu_code+"^"+grp_code+"^"+sub_grp_code);
	} else {
		arr_user_data = jQuery.grep(arr_user_data, function(value) {
			return value != menu_code+"^"+sub_menu_code+"^"+grp_code+"^"+sub_grp_code;
		});		
	}
	//console.log(arr_user_data);
}
		...
		bootbox.confirm({
		    ...
		    callback: function(result){
		    	 if(result == true) {
		    		 ...
		    		 for (var i = 0; i < arr_user_data.length; i++) {
		    			 var temp_data = arr_user_data[i].split("^");
		    			 
		    			 if(temp_data[0] != "N/A") {
		    				 if (arr_menu.indexOf(temp_data[0]) == -1) {
		    					 arr_menu.push(temp_data[0]);
		    				 }  
		    			 }
		    			 if(temp_data[1] != "N/A") {
		    				 if (arr_sub_menu.indexOf(temp_data[1]) == -1) {
		    					 arr_sub_menu.push(temp_data[1]);
		    				 }  
		    			 }
		    			 if(temp_data[2] != "N/A") {
		    				 if (arr_grp.indexOf(temp_data[2]) == -1) {
		    					 arr_grp.push(temp_data[2]);
		    				 }  
		    			 }
		    			 if(temp_data[3] != "N/A") {
		    				 if (arr_sub_grp.indexOf(temp_data[3]) == -1) {
		    					 arr_sub_grp.push(temp_data[3]);
		    				 }  
		    			 }
		    		 }
		    		 ...
		    		 for (var i = 0; i < arr_sub_menu.length; i++) {
		    			 subMenuCode += arr_sub_menu[i]+",";
		    		 }
		    		 for (var i = 0; i < arr_grp.length; i++) {
		    			 grpCode += arr_grp[i]+",";
		    		 }
		    		 for (var i = 0; i < arr_sub_grp.length; i++) {
		    			 subGrpCode += arr_sub_grp[i]+",";
		    		 }
		    		 
		    		 menuCode = menuCode.substring(0, (menuCode.length-1));
		    		 subMenuCode = subMenuCode.substring(0, (subMenuCode.length-1));
		    		 grpCode = grpCode.substring(0, (grpCode.length-1));
		    		 subGrpCode = subGrpCode.substring(0, (subGrpCode.length-1));
	    			 ...
	    			 if(strUsrLvl == "ST") {
	    				 json_param_set["userlvlCode"] = GLOBAL_MAP_ID;
	    			 } else if(strUsrLvl == "DT") {
	    				 json_param_set["userlvlCode"] = $("#selDist option:selected").val();
	    			 } else if(strUsrLvl == "BK") {
	    				 json_param_set["userlvlCode"] = $("#selBlock option:selected").val();
	    			 } else if(strUsrLvl == "GP") {
	    				 json_param_set["userlvlCode"] = $("#selGP option:selected").val();
	    			 } else if(strUsrLvl == "MP") {
	    				 json_param_set["userlvlCode"] = $("#selMPA option:selected").val();
	    			 }		    			 		    			 
			
	    			 json_param_set["menuCode"] = menuCode;
	    			 json_param_set["subMenuCode"] = subMenuCode;
	    			 json_param_set["grpCode"] = grpCode;
	    			 json_param_set["subGrpCode"] = subGrpCode;	    			 
...
function popUserForDelete() {
	showModal();
	$('#sel_user_del').html('');
	$('#sel_user_del').append('<option disabled="disabled" selected="true" value="">Select User</option>');
	...
	$.ajax({
		  ...
		  success: function(response) {			  			 
			  if (response == "N/A") {
					...
					//console.log(obj_response);
					for ( var i = 0; i < obj_response.length; i++) {
						$('#sel_user_del').append('<option value="'+obj_response[i][0]+'">'+obj_response[i][1]+'</option>');
	  		        }					
	  		        hideModal();
				}
		  },
...
$("#btnDltUsrDelete").click(function() {
	...
	if($("#sel_user_del option:selected").val() == "") {
	...
	bootbox.confirm({
	    ...
	    callback: function(result){
	    	if(result == true) {
	    		var json_param_set = {};	    		 
				json_param_set["user_id"] = $("#sel_user_del option:selected").val();
				json_param_set["loginusrid"] = GLOBAL_LOGIN_USER_ID;
    			...
    			$.ajax({
    			    ...
    			    success: function(response){
    			    	...
    			    	if (obj_response.status == "1") {
    						...
    						bootbox.alert('Fail to delete User.');
    						hideModal();
    			    		return;
    					}
    			    },
...
$('.mod_type').on('change', function() {
	...
	$('#sel_user_mod').html('');
	$('#sel_user_mod').append('<option disabled="disabled" selected="true" value="">Select User</option>');
});
...
function popDistMod() {
	$('#sel_dist_mod').html('');
	$('#sel_dist_mod').append('<option disabled="disabled" selected="true" value="">Select District</option>');
	
	$('#sel_add_bk_dist_mod').html('');
	$('#sel_add_bk_dist_mod').append('<option disabled="disabled" selected="true" value="">Select District</option>');
	//# Get Unique District
...
function popBlockMod() {
	...
	$('#sel_add_bk_bk_mod')
	.empty()
	.append('<option disabled="disabled" selected="true" value="">Select Block</option>');	
...
function clearDataAddBlock() {
	...
	$("select#sel_add_bk_dist_mod").prop('selectedIndex', 0);
	$('#sel_add_bk_bk_mod').html('');
	$('#sel_add_bk_bk_mod').append('<option disabled="disabled" selected="true" value="">Select Block</option>');
